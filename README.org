...
The persistent =Threads= modeline entry opens a native, touch-sized thread list.
It renders the validated local catalog immediately, refreshes it asynchronously,
and exposes visible refresh or retry rows.  Each two-line row opens its exact
thread with mouse-1 or RET.  =C-c C-a t= offers the filtered catalog completion
chooser when that catalog contains threads, and otherwise opens the native list.
Both surfaces share the same complete thread, repository, and harness catalog.
