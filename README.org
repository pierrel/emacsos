...
=POST /chat= from the phone's CHAT page.  It runs assist's agent in
process (no CLI shellout) and returns the agent's reply for rendering
in the phone's =*chat*= buffer.

** Running with vs. without an LLM endpoint

The server has two modes:

- *LLM mode* (default when =ASSIST_MODEL_URL= is set): each /chat
  goes through =assist.Thread.message()=, which uses the configured
  model and any built-in skills / sub-agents.  Responses can take up
  to ~5 minutes; the client timeout in =chat.el= matches.
- *Echo mode* (default when =ASSIST_MODEL_URL= is unset, or
  =EMACSOS_LLM_DISABLED=1=): the server skips assist and replies
  =echo: <message>=.  Same shape, no model dependency; lets =make
  smoke= run on any clean checkout.

Either mode fires the back-channel =(message …)= on the phone via
=emacsclient= so the user sees the chat completed.
...
=make smoke= spins a dockerized phone simulation, starts the server,
and exercises both the curl path (server in isolation) and the
=chat.el= path (full client surface).  Set =ASSIST_MODEL_URL= to
exercise LLM mode; leave unset for echo-only.
