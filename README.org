...
Each =/chat= request goes through =assist.Thread.message()=, which
uses the configured model plus assist's built-in skills and
sub-agents (research, context, critique, fact-check).  Real
responses can take up to ~5 minutes for research-shaped questions;
the client timeout in =chat.el= matches.  The post-response
back-channel fires =(message "agent: <reply prefix>")= on the
phone via =emacsclient= so the user sees the round trip completed.
