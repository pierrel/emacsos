...
The chat panel POSTs to =emacos-chat-server-url= (default =http://localhost:8765/chat=). For pure UI/layout work no server is needed; to make the simulator actually reach an agent, run a [[*Chat Server][Chat Server]] and point that variable at it.
...
emacsclient -f ./server-file -e '(emacos--tap-return)'       # exit minibuffer; otherwise open safe object, or newline
...
emacsclient -f ./server-file -e '(emacos--chat-show-top-buffer)'                    # bring chat to the editor window
#+end_src
...
- *Emacs interactive commands are deterministic primitives.* A concrete
  ...
  loop and no callback into the agent. =(emacos-call "+14155550123")= stages
...
- *Skills are elisp recipes.* A skill does not add a bespoke tool; it tells the
  ...
  (=(emacos-call "+1…")= stages a proposal) or *change/extend* functionality (define a command,
...
command's concrete arguments. "call Ana" is agent work; =(emacos-call
...
stages it via =emacos-call=; two local phone actions perform the dial.) New
