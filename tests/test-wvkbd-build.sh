...
artifact=$build_dir/wvkbd-emacsos
benchmark=$build_dir/bench-glide
scratch=$(mktemp -d)
trap 'rm -rf -- "$scratch"' EXIT HUP INT TERM
build_image=$(awk -F "'" '$1 == "image=" && NF == 3 { print $2 }' \
    "$repo_dir/deploy/pinephone/build-wvkbd-emacsos.sh")
...
for script in \
    "$repo_dir/deploy/pinephone/build-wvkbd-emacsos.sh" \
    "$repo_dir/deploy/pinephone/bench-wvkbd-emacsos.sh" \
    "$repo_dir/deploy/pinephone/install-wvkbd-emacsos.sh" \
    "$repo_dir/deploy/pinephone/install-wvkbd-emacsos-root" \
    "$repo_dir/deploy/pinephone/wvkbd-transaction-root"
do
sh -n "$script"
done
make -s -n -C "$repo_dir" wvkbd-phone-bench PINEPHONE_HOST=phone-wg |
    grep -F 'bench-wvkbd-emacsos.sh' >/dev/null
grep -F 'timeout 60' "$repo_dir/deploy/pinephone/bench-wvkbd-emacsos.sh" >/dev/null
grep -F 'wvkbd-bench.XXXXXX' "$repo_dir/deploy/pinephone/bench-wvkbd-emacsos.sh" >/dev/null
grep -F "&& file" "$repo_dir/deploy/pinephone/bench-wvkbd-emacsos.sh" >/dev/null
...
grep -F '/usr/local/share/licenses/wvkbd-emacsos/wordninja.txt' "$transaction" >/dev/null
grep -F 'wvkbd-transaction-root' "$repo_dir/deploy/pinephone/openrc-manifest.sha256" >/dev/null
grep -F 'WVKBD_NOTICE_STAGE' "$repo_dir/deploy/pinephone/install-wvkbd-emacsos.sh" >/dev/null
revision=$(cat "$repo_dir/deploy/pinephone/wvkbd-revision")
printf '%s\n' "$revision" | grep -Eq '^[0-9a-f]{40}$'
make_output=$(make -C "$wvkbd_dir" -n BIN=wvkbd-emacsos LAYOUT=mobintl)
printf '%s\n' "$make_output" | grep -F ' -o wvkbd-emacsos ' >/dev/null
grep -F 'target=/usr/local/bin/wvkbd-emacsos' "$transaction" >/dev/null
...
WVKBD_REPO_DIR=$scratch/wvkbd WVKBD_BUILD_DIR=$scratch/archive-hostile \
    "$repo_dir/deploy/pinephone/build-wvkbd-emacsos.sh" >/dev/null
[ "$(sha256sum "$scratch/archive-hostile/wvkbd-emacsos" | awk '{print $1}')" = "$baseline_keyboard" ]
...
WVKBD_BUILD_DIR=$build_dir "$repo_dir/deploy/pinephone/build-wvkbd-emacsos.sh"
...
grep -F 'recovery helper is not installed safely' \
    "$repo_dir/deploy/pinephone/install-wvkbd-emacsos-root" >/dev/null
...
docker run --rm --cap-add SYS_PTRACE \
    ...
    -v "$repo_dir/deploy/pinephone/install-wvkbd-emacsos-root:/install-root:ro" \
...
EOF
        cc -s -o /usr/local/bin/wvkbd-emacsos /tmp/keyboard.c
...
EOF
        ...
        install -m 0755 /install-root /usr/local/sbin/install-wvkbd-emacsos
        for invalid in missing unsafe one-hook; do
            ...
            if SUDO_USER=user /usr/local/sbin/install-wvkbd-emacsos >/dev/null 2>&1; then exit 1; fi
        done
        printf "%s\n%s\n" \
...
case $2 in
 ...
 start|restart) for fd in 6 7 8 9; do case $(readlink "/proc/$$/fd/$fd" 2>/dev/null || true) in /run/wvkbd-emacos-install.lock|/run/wvkbd-emacsos-install.lock|/run/emacsos-openrc-install.lock|/run/emacsos-openrc-boot-mode.lock) exit 1 ;; esac; done; /usr/local/sbin/emacsos-wvkbd-transaction prepare-start || exit 1; mkdir -p /run/emacsos-ui "$cgroup"; rm -f /run/emacsos-ui/ready /run/emacsos-ui/service /tmp/startpost-session-verified; [ -f /run/emacsos-ui/pid ] && kill "$(cat /run/emacsos-ui/pid)" 2>/dev/null || true; su -s /bin/sh emacsos-lab -c "/usr/local/bin/wvkbd-emacsos --mod-swipe -H 300 -L 300 >/dev/null 2>&1 & echo \$!" >/run/emacsos-ui/pid; chown emacsos-lab:emacsos-lab /run/emacsos-ui/pid; cat /run/emacsos-ui/pid >"$cgroup/cgroup.procs"; printf "populated 1\\n" >"$cgroup/cgroup.events"; : >"$cgroup/cgroup.kill"; printf "ready\\n" >/run/emacsos-ui/ready; chown emacsos-lab:emacsos-lab /run/emacsos-ui/ready; chmod 0600 /run/emacsos-ui/ready; [ ! -e /run/emacsos-ui/service ]; /usr/local/sbin/emacsos-wvkbd-transaction verify-start || exit 1; : >/tmp/startpost-session-verified; [ ! -e /tmp/suppress-service-marker ] || { rm -f /tmp/suppress-service-marker; exit 0; }; : >/run/emacsos-ui/service ;;
...
EOF
        ...
        new=$(sha256sum /home/user/.cache/wvkbd-emacsos.ABC123 | awk "{print \$1}")
        ...
        for invalid_digest in keyboard notice; do
            ...
            if SUDO_USER=user WVKBD_STAGE=/home/user/.cache/wvkbd-emacsos.ABC123 WVKBD_SHA256=$candidate_new WVKBD_NOTICE_STAGE=/home/user/.cache/wvkbd-notice.ABC123 WVKBD_NOTICE_SHA256=$candidate_notice /usr/local/sbin/emacsos-wvkbd-transaction activate 2>/tmp/wvkbd-digest-error; then exit 1; fi
        ...
        rm -f /usr/local/bin/wvkbd-emacsos
        if /usr/local/sbin/emacsos-wvkbd-transaction verify-current 2>/dev/null; then exit 1; fi
        install -o root -g root -m 0755 /home/user/.cache/wvkbd-emacsos.ABC123 /usr/local/bin/wvkbd-emacsos
        install -o root -g root -m 0755 /bin/true /usr/local/bin/wvkbd-emacsos
        if /usr/local/sbin/emacsos-wvkbd-transaction verify-current 2>/dev/null; then exit 1; fi
        install -o root -g root -m 0755 /home/user/.cache/wvkbd-emacsos.ABC123 /usr/local/bin/wvkbd-emacsos
        : >/tmp/openrc.emacsos-ui/cgroup.procs
        if /usr/local/sbin/emacsos-wvkbd-transaction verify-current 2>/dev/null; then exit 1; fi
        ...
        for corrupt in mode link hardlink; do
            ...
            install -m 0755 /home/user/.cache/wvkbd-emacsos.ABC123 /usr/local/bin/wvkbd-emacsos
        ...
        su -s /bin/sh emacsos-lab -c "/usr/local/bin/wvkbd-emacsos --mod-swipe -H 300 -L 300 >/dev/null 2>&1 & echo \$!" >/tmp/extra-keyboard-pid
        ...
        cc -s -o /home/user/.cache/wvkbd-emacsos.DEF456 /tmp/keyboard.c
        printf notice-two >/home/user/.cache/wvkbd-notice.DEF456
        chown user:user /home/user/.cache/wvkbd-emacsos.DEF456 /home/user/.cache/wvkbd-notice.DEF456
        chmod 0600 /home/user/.cache/wvkbd-emacsos.DEF456 /home/user/.cache/wvkbd-notice.DEF456
        third=$(sha256sum /home/user/.cache/wvkbd-emacsos.DEF456 | awk "{print \$1}")
        third_notice=$(sha256sum /home/user/.cache/wvkbd-notice.DEF456 | awk "{print \$1}")
        touch /tmp/suppress-service-marker
        if SUDO_USER=user WVKBD_STAGE=/home/user/.cache/wvkbd-emacsos.DEF456 WVKBD_SHA256=$third WVKBD_NOTICE_STAGE=/home/user/.cache/wvkbd-notice.DEF456 WVKBD_NOTICE_SHA256=$third_notice /usr/local/sbin/emacsos-wvkbd-transaction activate; then exit 1; fi
        [ "$(sha256sum /usr/local/bin/wvkbd-emacsos | awk "{print \$1}")" = "$new" ]
        [ ! -e /var/lib/emacsos-wvkbd-transaction/state ]
        ...
        cp /home/user/.cache/wvkbd-emacsos.DEF456 /usr/local/bin/wvkbd-emacsos
        cp /usr/local/bin/wvkbd-emacsos /tmp/pending-candidate
        cp /home/user/.cache/wvkbd-emacsos.ABC123 /var/lib/emacsos-wvkbd-transaction/previous
        chmod 0600 /var/lib/emacsos-wvkbd-transaction/previous
        ...
        cp /usr/local/share/licenses/wvkbd-emacsos/wordninja.txt /var/lib/emacsos-wvkbd-transaction/previous-notice
        ...
        cp /usr/local/bin/wvkbd-emacsos /var/lib/emacsos-wvkbd-transaction/previous
        chmod 0600 /var/lib/emacsos-wvkbd-transaction/previous
        owner=$$
        ...
        for prior in 0 1; do
            for installed in old new; do
                ...
                chmod 0600 /var/lib/emacsos-wvkbd-transaction/previous
                if [ "$prior" = 1 ]; then
                    install -d -m 0755 /usr/local/share/licenses/wvkbd-emacsos
                    printf old-notice >/usr/local/share/licenses/wvkbd-emacsos/wordninja.txt
                    cp /usr/local/share/licenses/wvkbd-emacsos/wordninja.txt /var/lib/emacsos-wvkbd-transaction/previous-notice
                    chown root:root /var/lib/emacsos-wvkbd-transaction/previous-notice
                    chmod 0600 /var/lib/emacsos-wvkbd-transaction/previous-notice
                else
                    rm -f /usr/local/share/licenses/wvkbd-emacsos/wordninja.txt
                fi
                ...
                chown root:root /var/lib/emacsos-wvkbd-transaction/state
                chmod 0600 /var/lib/emacsos-wvkbd-transaction/state
                if [ "$installed" = new ]; then install -m 0755 /home/user/.cache/wvkbd-emacsos.ABC123 /usr/local/bin/wvkbd-emacsos; fi
                EMACSOS_WVKBD_CANDIDATE_SHA256="$new" /usr/local/sbin/emacsos-wvkbd-transaction prepare-start
                [ "$(sha256sum /usr/local/bin/wvkbd-emacsos | awk "{print \$1}")" = "$old" ]
...
EOF
        ...
        for target_case in old unknown; do
            ...
            rm -rf /var/lib/emacsos-wvkbd-transaction
            install -d -o root -g root -m 0700 /var/lib/emacsos-wvkbd-transaction
            case $target_case in old) cc -s -o /usr/local/bin/wvkbd-emacsos /tmp/keyboard.c; chmod 0755 /usr/local/bin/wvkbd-emacsos ;; unknown) install -m 0755 /bin/true /usr/local/bin/wvkbd-emacsos ;; esac
            printf "version=1\\nphase=committed\\nold_sha=%s\\nnew_sha=%s\\nprior_notice_present=0\\n" "$old" "$new" >/var/lib/emacsos-wvkbd-transaction/state
            chown root:root /var/lib/emacsos-wvkbd-transaction/state
            chmod 0600 /var/lib/emacsos-wvkbd-transaction/state
        ...
        for point in before-pending after-pending after-notice-install \
            temporary-/usr/local/bin/wvkbd-emacsos renamed-/usr/local/bin/wvkbd-emacsos \
            installed-/usr/local/bin/wvkbd-emacsos after-candidate-rename \
            ...
            for prior in present absent; do
                ...
                if [ "$prior" = present ]; then
                    install -d -m 0755 /usr/local/share/licenses/wvkbd-emacsos
                    install -m 0644 /home/user/.cache/wvkbd-notice.ABC123 /usr/local/share/licenses/wvkbd-emacsos/wordninja.txt
                else
                    rm -f /usr/local/share/licenses/wvkbd-emacsos/wordninja.txt
                fi
                cc -s -o /usr/local/bin/wvkbd-emacsos /tmp/keyboard.c
                chmod 0755 /usr/local/bin/wvkbd-emacsos
                old=$(sha256sum /usr/local/bin/wvkbd-emacsos | awk "{print \$1}")
                if SUDO_USER=user WVKBD_STAGE=/home/user/.cache/wvkbd-emacsos.ABC123 WVKBD_SHA256=$new WVKBD_NOTICE_STAGE=/home/user/.cache/wvkbd-notice.ABC123 WVKBD_NOTICE_SHA256=$notice WVKBD_TEST_KILL_AT=$point /transaction-test activate; then exit 1; fi
                case $point in after-committed|cleanup-*) expected=$new ;; *) expected=$old ;; esac
                /transaction-test prepare-start
                [ "$(sha256sum /usr/local/bin/wvkbd-emacsos | awk "{print \$1}")" = "$expected" ]
                /usr/sbin/rc-service emacsos-ui start
                /transaction-test verify-start
                [ ! -e /var/lib/emacsos-wvkbd-transaction/state ]
                [ "$(sha256sum /usr/local/bin/wvkbd-emacsos | awk "{print \$1}")" = "$expected" ]
                [ "$(for proc in /proc/[0-9]*; do [ "$(stat -c %U "$proc" 2>/dev/null || true)" = emacsos-lab ] || continue; actual=$(tr "\000" " " <"$proc/cmdline" | sed "s/ $//"); [ "$actual" = "/usr/local/bin/wvkbd-emacsos --mod-swipe -H 300 -L 300" ] && printf x; done | wc -c)" -eq 1 ]
            done
        done
