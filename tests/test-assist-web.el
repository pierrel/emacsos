...
(ert-deftest test-assist-web-new-thread-fetches-catalog-with-no-existing-thread ()
  (let ((catalog '((threads . nil)
                   (repositories . (((repo_key . "repo-key") (label . "Assist"))))
                   (harnesses . (((key . "deepagents") (label . "Deep Agents")))))))
...
(ert-deftest test-assist-web-has-global-entry-and-thread-local-shortcuts ()
  (should (eq (lookup-key (current-global-map) (kbd "C-c C-t"))
              #'emacos-assist-web-open-thread))
  (should (eq (lookup-key (current-global-map) (kbd "C-c C-n"))
              #'emacos-assist-web-new-thread))
  (should (eq (lookup-key emacos-assist-web-mode-map (kbd "C-c C-d"))
              #'emacos-assist-web-show-diff))
  (should (eq (lookup-key emacos-assist-web-mode-map (kbd "C-c C-f"))
              #'emacos-assist-web-show-files))
  (should (eq (lookup-key emacos-assist-web-mode-map (kbd "C-c C-p"))
              #'emacos-assist-web-show-pins))
  (should (eq (lookup-key emacos-assist-web-mode-map (kbd "C-c C-a"))
