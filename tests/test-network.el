...
(ert-deftest test-net-action-overflow-reports-bound-exactly-once ()
  (let ((emacsos-net--action-processes nil)
        (emacsos-net--settle-generation 0)
        (emacsos-net--settle-pending nil)
    ...
    (cl-letf (((symbol-function 'make-process)
              ...
              ((symbol-function 'process-get)
               (lambda (_proc key)
                 (eq key 'emacsos-net-action-overflow)))
              ((symbol-function 'emacsos-net--discard-reader) #'ignore)
              ((symbol-function 'run-with-timer) (lambda (&rest _) 'timer)))
      (emacsos-net--action
       '("radio" "wifi" "on")
       (lambda (success detail) (push (list success detail) delivered)))
      (with-current-buffer action-buffer (insert "partial diagnostic"))
      (funcall sentinel 'action-process "killed")
      (funcall sentinel 'action-process "killed again"))
    (should (equal delivered
                   '((nil "network action response is too large"))))
    (should-not (buffer-live-p action-buffer))))

(ert-deftest test-net-reload-terminates-tracked-and-pre-upgrade-actions ()
  (let ((emacsos-net--action-processes '(tracked))
        killed changed-sentinel)
    (cl-letf (((symbol-function 'process-list)
               (lambda () '(tracked legacy unrelated)))
              ((symbol-function 'process-name)
               (lambda (proc)
                 (pcase proc
                   ('tracked "emacsos-net-act")
                   ('legacy "emacsos-net-act<1>")
                   (_ "other"))))
              ((symbol-function 'process-live-p) (lambda (_) t))
              ((symbol-function 'process-id)
               (lambda (proc) (if (eq proc 'tracked) 10 11)))
              ((symbol-function 'signal-process) #'ignore)
              ((symbol-function 'delete-process)
               (lambda (proc) (push proc killed)))
              ((symbol-function 'set-process-sentinel)
               (lambda (&rest _) (setq changed-sentinel t))))
      (emacsos-net--discard-actions))
    (should-not emacsos-net--action-processes)
    (should (equal (sort killed (lambda (a b)
