    ...
    (let ((default-directory "/ssh:thinky:"))
      (cl-letf (((symbol-function 'process-attributes)
                ...
                ((symbol-function 'file-symlink-p)
                 (lambda (_) "/usr/local/bin/wvkbd-emacsos"))
                ((symbol-function 'insert-file-contents-literally)
                 (lambda (_)
                   (insert (concat "/usr/local/bin/wvkbd-emacsos\0"
...
(ert-deftest emacsos-openrc-ui-session-parent-validates-exact-process ()
  (let (requested)
    (cl-letf (((symbol-function 'process-attributes)
               (lambda (pid)
                 (should (= pid 42))
              ...
              ((symbol-function 'insert-file-contents-literally)
               (lambda (path)
                 (setq requested path)
                 (insert (concat "/bin/sh\0"
                                 "/usr/local/share/emacsos-openrc/session\0")))))
      (should (emacsos-pinephone--ui-session-parent-p 42))
      (should (equal requested "/proc/42/cmdline")))))

(ert-deftest emacsos-openrc-ui-restart-refuses-unsaved-file-buffer ()
  (let ((buffer (generate-new-buffer " *test-unsaved-file*"))
        signaled)
    (unwind-protect
        (progn
          (with-current-buffer buffer
            (setq buffer-file-name "/tmp/test-unsaved-file")
            (insert "unsaved"))
          (cl-letf (((symbol-function 'buffer-list) (lambda () (list buffer)))
                    ((symbol-function 'signal-process)
                     (lambda (&rest _) (setq signaled t))))
            (should-error (emacsos-pinephone-restart-ui-session)
                          :type 'user-error)
            (should-not signaled)))
      (kill-buffer buffer))))

(ert-deftest emacsos-openrc-ui-restart-signals-validated-session-parent ()
  (let (signaled message-text)
    (cl-letf (((symbol-function 'buffer-list) (lambda () nil))
              ((symbol-function 'process-attributes)
               (lambda (pid)
                 (should (= pid (emacs-pid)))
                 '((ppid . 42))))
              ((symbol-function 'emacsos-pinephone--ui-session-parent-p)
               (lambda (pid) (= pid 42)))
      ...
      (should (equal signaled '(42 SIGTERM)))
      (should (equal message-text
                     "Restarting the phone UI; transient buffers will close...")))))
...
(ert-deftest emacsos-openrc-wifi-ordinary-exit-drains-terminal-result ()
  (let (stdout stderr installed-sentinel scheduled output-drained delivered
    ...
    (cl-letf (((symbol-function 'make-process)
              ...
              ((symbol-function 'accept-process-output)
               (lambda (_process _seconds)
                 (setq output-drained t)
                 (with-current-buffer stdout (insert "connected\n"))))
              ((symbol-function 'process-status)
               (lambda (_) (if terminal 'exit 'run)))
              ((symbol-function 'process-buffer) (lambda (_) stdout))
              ((symbol-function 'process-exit-status) (lambda (_) 0)))
      (should (equal
               (emacsos-pinephone-wifi-operation
                'open "Cafe network" nil
                (lambda (result)
                  (setq delivered result)
                  (cl-incf completions)))
               "pending: Wi-Fi connection requested"))
      ...
      (should-not output-drained)
      (should (zerop (buffer-size stdout)))
      (setq terminal t)
      (funcall installed-sentinel 'wifi-process "finished")
      (should output-drained)
      (should (= completions 1))
      (should (equal delivered "connected"))
      (should-not (buffer-live-p stdout))
      (should-not (buffer-live-p stderr))
      (funcall installed-sentinel 'wifi-process "finished")
      (should (= completions 1)))))
