...
(ert-deftest test-os-thread-commands-keep-list-and-completion-distinct ()
  "The Assist prefix has separate native-list and completion commands."
  (let (opened)
    (cl-letf (((symbol-function 'emacsos-assist-web-open-thread)
               (lambda () (interactive) (setq opened t)))
              ((symbol-function 'emacsos-assist-web-show-thread-list)
               (lambda () (interactive)
                 (ert-fail "completion command must not open the native list"))))
      (emacsos-command-open-thread))
    (should opened))
  (let (opened)
    (cl-letf (((symbol-function 'emacsos-assist-web-show-thread-list)
               (lambda () (interactive) (setq opened t))))
      (emacsos-command-list-threads))
    (should opened)))

(ert-deftest test-os-global-command-prefix-owns-portable-actions ()
  (should emacsos-command-mode)
  (dolist (binding '(("C-c a l" . emacsos-command-list-threads)
                     ("C-c a t" . emacsos-command-open-thread)
                     ("C-c a n" . emacsos-command-new-thread)
                     ("C-c a r" . emacsos-assist-web-refresh-threads)
                     ("C-c a f" . emacsos-assist-new-file)
                     ("C-c e c" . emacsos--chat-show-top-buffer)
                     ("C-c e d" . emacsos-call)
                     ("C-c e m" . emacsos-sms-chat-catalog)
                     ("C-c e s" . emacsos-send-message)
                     ("C-c e w" . emacsos-net-show)
                     ("C-c e h" . emacsos-open-command-reference)))
    (should (eq (key-binding (kbd (car binding))) (cdr binding)))))

(ert-deftest test-os-legacy-project-bindings-are-not-retained ()
  (dolist (key '("C-c e t" "C-c e n" "C-c e r" "C-c e f"
                 "C-c C-a n" "C-c C-a t"))
    (should-not (memq (key-binding (kbd key))
                      '(emacsos-command-list-threads emacsos-command-open-thread
                        emacsos-command-new-thread emacsos-assist-web-refresh-threads
                        emacsos-assist-new-file)))))
