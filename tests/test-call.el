...
(ert-deftest emacsos-call-sms-observer-is-independent-bounded-and-owner-checked ()
  (let ((emacsos-call--current-owner ":1.mm")
        (emacsos-call--owner-generation 4)
        (emacsos-call-wake-function nil)
        (emacsos-call-sms-added-functions nil)
        seen)
    (setq emacsos-call-sms-added-functions
          (list (lambda (&rest _) (error "isolated"))
                (lambda (owner generation path)
                  (setq seen (list owner generation path)))))
    (cl-letf (((symbol-function 'emacsos-call--event-owner) (lambda () ":1.mm")))
      (emacsos-call--on-sms-added "/org/freedesktop/ModemManager1/SMS/12" t)
      (should (equal seen '(":1.mm" 4
                            "/org/freedesktop/ModemManager1/SMS/12")))
      (setq seen nil)
      (emacsos-call--on-sms-added
       (concat "/org/freedesktop/ModemManager1/SMS/" (make-string 200 ?1)) t)
      (should-not seen))
    (setq emacsos-call--current-owner nil)
    (cl-letf (((symbol-function 'emacsos-call--event-owner) (lambda () ":1.mm")))
      (emacsos-call--on-sms-added "/org/freedesktop/ModemManager1/SMS/12" t)
      (should-not seen))))

(ert-deftest emacsos-call-owner-bootstrap-cannot-overwrite-reentrant-transition ()
  (let ((emacsos-call--watcher-handles nil)
        (emacsos-call--owner-watch-handle nil)
        (emacsos-call--call-added-handle nil)
        (emacsos-call--state-handle nil)
        ...
        (emacsos-call--owner-generation 0)
        (emacsos-call-owner-changed-functions nil))
    (cl-letf (((symbol-function 'emacsos-call--register-signal-bounded)
               (lambda (&rest _) (gensym "watch")))
              ((symbol-function 'emacsos-call--modem-manager-owner)
               (lambda ()
      ...
      (should (= emacsos-call--owner-generation 1)))))

(ert-deftest emacsos-call-stale-owner-transition-cannot-replace-current-authority ()
  (let ((emacsos-call--current-owner ":1.current")
        (emacsos-call--owner-watch-handle 'owner-watch)
        (emacsos-call--owner-generation 7))
    (emacsos-call--on-owner-changed
     "org.freedesktop.ModemManager1" ":1.stale" ":1.attacker")
    (should (equal emacsos-call--current-owner ":1.current"))
    (should (= emacsos-call--owner-generation 7))))

(ert-deftest emacsos-call-unregistered-owner-callback-cannot-restore-authority ()
  (let ((emacsos-call--current-owner nil)
        (emacsos-call--owner-watch-handle nil)
        (emacsos-call--owner-generation 7))
    (emacsos-call--on-owner-changed
     "org.freedesktop.ModemManager1" "" ":1.stale")
    (should-not emacsos-call--current-owner)
    (should (= emacsos-call--owner-generation 7))))
